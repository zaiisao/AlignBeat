...
class ScaleHead(nn.Module):
    ...
    def __init__(self, d_model: int, hidden: int = 128, b_min: float = B_MIN):
        super().__init__()
        self.b_min = b_min
        self.net = nn.Sequential(
            nn.Linear(d_model, hidden), nn.GELU(), nn.Linear(hidden, 1))
...
class PhaseSelectionHead(nn.Module):
    ...
    def __init__(self, feature_size=512, reduced_dim=256, hidden_size=128,
                 window_seconds=30.0,
                 phase_attention_layers=1, phase_attention_heads=4,
                 phase_attention_final_norm=False,
        ...
        self.precision_head = ScaleHead(reduced_dim, hidden_size, b_min)
    ...
    def forward(self, x, epoch=None):
        ...
        tilde_z = z if self.candidate_attention is None else self.candidate_attention(z)
...
class PhaseTimeHead(nn.Module):
    ...
    def __init__(self, input_dim, num_candidates,
                 ...
                 downsample_stages=None, reduced_dim=256, hidden_size=128,
                 phase_attention_layers=1, phase_attention_heads=4,
                 phase_attention_final_norm=False, warmup_epochs=5):
        super().__init__()
