...
Vendored from Beat This! (CPJKU/beat_this, beat_this/dataset/mmnpz.py), MIT licence,
Copyright (c) 2024 Institute of Computational Perception, JKU Linz, Austria. Unmodified
apart from this header.

Why this is here rather than np.load: an .npz member is a zip entry, so np.load decodes
it WHOLE. The corpus stores one array per track, and a track is minutes long while we
train on 30-second crops -- on harmonix (median 2.8 min) that decoded 8264 frames to use
1500, throwing away 82% of the work, on the dataloader's critical path. This maps the
archive once and returns views into it, so slicing a crop faults in only that crop's
pages. Measured effect on the corpus: harmonix reads ~14.4 ms -> ~2.6 ms.
"""
